...
const { calculateSessionXP, updateStreak, checkLevelUp, applyStatGrowth } = require(path.join(libDir, 'xp'));
const { checkAchievements, updateProgress, updateTestDrivenStreak } = require(path.join(libDir, 'achievements'));
const { generateEntry, appendJournal } = require(path.join(libDir, 'journal'));
...
async function main() {
  ...
  const soul = loadSoul();
  if (!soul) {
    process.exit(0); // No soul file, nothing to do
  }
  ...
  // Skip empty sessions (no tool calls = probably just opened and closed)
  ...
  // 10. Update achievement progress
  updateProgress(soul);

  // 11. Save soul
  saveSoul(soul);

  // 12. Generate and append journal entry
  const entry = generateEntry(session, soul, newAchievements, totalXP, levelResult);
  appendJournal(entry);

  // 13. Print summary to stderr
  const species = getSpeciesInfo(soul);
  const emoji = species?.emoji || '🐾';
  const lines = [
    `${emoji} Session complete! +${totalXP.toLocaleString('en-US')} XP | Level ${soul.progression.level} | ${soul.progression.totalXP.toLocaleString('en-US')} XP total`,
  ];
  ...
  for (const a of newAchievements) {
    lines.push(`   🏆 Achievement: ${a.name} — ${a.description} (+${a.xp.toLocaleString('en-US')} XP)`);
  }
...
function updateFamiliarity(soul, session) {
  ...
  if (!soul.familiarity[projectDir]) {
    soul.familiarity[projectDir] = { sessions: 0, files: {} };
  }
