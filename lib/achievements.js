...
const ACHIEVEMENTS = [
  ...
  },
  {
    ...
    trigger: (session) => session.fileEdits >= 5, // conservative: any productive session
    xp: 400, rarity: 'uncommon',
    // Will refine when we can detect context resets from transcript
  },
  ...
  },
  {
    ...
    trigger: (_s, soul) => {
      const nonHidden = ACHIEVEMENTS.filter(a => !a.hidden && a.id !== 'completionist');
      const earnedIds = new Set(soul.achievements.earned.map(e => e.id));
      return nonHidden.every(a => earnedIds.has(a.id));
    },
