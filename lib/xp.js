...
const {
  ...
  PRESTIGE_TERRAINS, PRESTIGE_MAX,
} = require('./constants');
...
function formatProgressBar(current, max, width = 20) {
...
// Prestige: reset level to 1, keep stats/achievements, change terrain
function canPrestige(soul) {
  return soul.progression.level >= 20 && (soul.progression.prestige || 0) < PRESTIGE_MAX;
}

function applyPrestige(soul) {
  const newPrestige = (soul.progression.prestige || 0) + 1;
  soul.progression.prestige = Math.min(newPrestige, PRESTIGE_MAX);
  soul.progression.level = 1;
  soul.progression.totalXP = 0;
  soul.progression.tier = 'hatchling';
  soul.progression.prestigedAt = soul.progression.prestigedAt || [];
  soul.progression.prestigedAt.push(new Date().toISOString());
  return {
    prestige: soul.progression.prestige,
    terrain: PRESTIGE_TERRAINS[soul.progression.prestige],
  };
}

function getPrestigeTerrain(soul) {
  const level = soul.progression.prestige || 0;
  return PRESTIGE_TERRAINS[Math.min(level, PRESTIGE_MAX)];
}
